...
def test_serialized_profile_contains_orchestration_fields() -> None:
    ...
    assert payload["system_prompt"] == PROVISIONING_AGENT_V1.content
    assert payload["allowed_tools"] == list(PROVISIONING_AGENT_V1.tool_allowlist)
    assert payload["required_first_calls"] == list(PROVISIONING_AGENT_V1.required_first_calls)
    assert payload["safety_rules"] == list(PROVISIONING_AGENT_V1.safety_rules)
    assert payload["prompt_version"] == "1.0.0"
    assert payload["prompt_hash"] == PROVISIONING_AGENT_V1.content_hash
