...
type SessionRow = {
  ...
  status: ChatSessionRecord["status"]; created_at: string; updated_at: string;
};
...
export class ChatPersistenceService {
  ...
  async listSessions(workspaceId: string, userId: string): Promise<ChatSessionRecord[]> {
    const result = await this.db.query<SessionRow>(
      `SELECT id, workspace_id, user_id, title, status, created_at, updated_at
       FROM provisr_state.chat_sessions
       WHERE workspace_id = $1 AND user_id = $2 AND status = 'active'
       ORDER BY updated_at DESC`, [workspaceId, userId],
    );
  ...
  async archiveSession(id: string, workspaceId: string, userId: string): Promise<void> {
    const result = await this.db.query(
      `UPDATE provisr_state.chat_sessions SET status = 'archived', updated_at = now()
       WHERE id = $1 AND workspace_id = $2 AND user_id = $3 AND status = 'active'`,
      [id, workspaceId, userId],
    );
  ...
  async listMessages(sessionId: string, workspaceId: string, userId: string): Promise<ChatMessageRecord[]> {
    ...
    const result = await this.db.query<{
      id: string; turn_id: string; role: ChatMessageRecord["role"]; content: string; created_at: string;
    }>(
      `SELECT id, turn_id, role, content, created_at FROM provisr_state.chat_messages
       WHERE session_id = $1 AND workspace_id = $2 ORDER BY created_at, id`, [sessionId, workspaceId],
    );
    return result.rows.map((row) => ({ id: row.id, turnId: row.turn_id, role: row.role, content: row.content, createdAt: row.created_at }));
  }
...
function toSession(row: SessionRow): ChatSessionRecord {
  return { id: row.id, workspaceId: row.workspace_id, userId: row.user_id, title: row.title, status: row.status, createdAt: row.created_at, updatedAt: row.updated_at };
}
