...
export class PlanningDispatchService {
  ...
  private async complete(turn: TurnRow, result: DispatchResponse): Promise<void> {
    ...
    try {
      ...
      await this.events.append({ sessionId: turn.session_id, workspaceId: turn.workspace_id, turnId: turn.turn_id, eventType: "planning.completed", payload: { runId: turn.run_id, manifestReady: Boolean(result.manifest_draft) } }, client);
      await client.query("COMMIT");
    } catch (error) { await client.query("ROLLBACK"); throw error; } finally { client.release(); }
  }
