...
describe("Controller Payload Validation & Strict Typing Integration Tests", () => {
  ...
  describe("ProvisioningRunsController Validation", () => {
    ...
    it("rejects GET /v1/runs with invalid status query filter", async () => {
      const res = await http(app)
        .get(`/v1/runs?status=UNKNOWN_STATUS`)
        .expect(400);
    ...
    it("rejects non-UUID path parameters across all run endpoints", async () => {
      await http(app).get(`/v1/runs/not-uuid`).expect(400);
      await http(app).post(`/v1/runs/not-uuid/confirm`).expect(400);
      await http(app).post(`/v1/runs/not-uuid/clarify`).expect(400);
      await http(app).post(`/v1/runs/not-uuid/cancel`).expect(400);
    });
  });
