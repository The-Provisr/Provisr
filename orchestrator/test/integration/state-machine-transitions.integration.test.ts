...
describe("State Machine Transitions Integration Tests", () => {
  ...
  function createTransactionalMockDb(initialRun: ProvisioningRun) {
    ...
    const auditLogs: Array<{ action: string; eventData: string; workspaceId: string; actorId: string; resourceId: string }> = [];
    const outboxEvents: Array<{ eventId: string; resourceId: string; eventType: string; payload: string }> = [];
    ...
    const poolClient = {
      query: vi.fn().mockImplementation(async (sql: string, params?: unknown[]) => {
        ...
        if (sql.includes("UPDATE provisr_state.provisioning_runs")) {
          const [newState, approvalStatus, policyDecision, id, wsId, expectedVer] = params as [
            RunState,
            string,
            string | null,
            string,
          ...
          if (
            ...
            currentRunState = {
              ...currentRunState,
              state: newState,
              approvalStatus: approvalStatus ?? currentRunState.approvalStatus,
              policyDecision: policyDecision ?? currentRunState.policyDecision,
  ...
  describe("Approval Edge Cases & Gating", () => {
    it("successfully approves a run in pending_approval state -> moves to pending_execution and marks approved", async () => {
      const initial = createMockRun({ state: "pending_approval", stateVersion: 8, approvalStatus: "pending" });
      const mockEnv = createTransactionalMockDb(initial);
      const service = new RunsService(mockEnv.db);

      const result = await service.handleApprovalEvent(runId, workspaceId, userId, "approved");
    ...
    it("successfully rejects a run in pending_approval state -> moves to pending_agent for redesign with reason", async () => {
      const initial = createMockRun({ state: "pending_approval", stateVersion: 8, approvalStatus: "pending" });
      const mockEnv = createTransactionalMockDb(initial);
      const service = new RunsService(mockEnv.db);

      const result = await service.handleApprovalEvent(
        runId,
        workspaceId,
        userId,
        "rejected",
        "Cost estimate exceeds monthly budget threshold",
      );
    ...
    it("rejects approval rejection when reason is missing or empty", async () => {
      const initial = createMockRun({ state: "pending_approval", stateVersion: 8 });
      const mockEnv = createTransactionalMockDb(initial);
      const service = new RunsService(mockEnv.db);

      await expect(
        service.handleApprovalEvent(runId, workspaceId, userId, "rejected"),
      ).rejects.toThrow("Reason is required when rejecting an approval");
    ...
    it("rejects approval decision when run is in a rejected state (pending_agent)", async () => {
      const rejectedRun = createMockRun({ state: "pending_agent", stateVersion: 9, approvalStatus: "rejected" });
      const mockEnv = createTransactionalMockDb(rejectedRun);
      const service = new RunsService(mockEnv.db);

      await expect(
        service.handleApprovalEvent(runId, workspaceId, userId, "approved"),
      ).rejects.toThrow(ConflictException);

      await expect(
        service.handleApprovalEvent(runId, workspaceId, userId, "approved"),
      ).rejects.toThrow("Cannot process approval decision for run in 'pending_agent' state (expected 'pending_approval')");
    });

    it("rejects approval decision on terminal or non-approval states", async () => {
      const invalidStates: RunState[] = ["completed", "failed", "cancelled", "executing", "received", "pending_confirmation"];
      ...
      for (const invalidState of invalidStates) {
        ...
        await expect(
          service.handleApprovalEvent(runId, workspaceId, userId, "approved"),
        ).rejects.toThrow(`Cannot process approval decision for run in '${invalidState}' state (expected 'pending_approval')`);
      }
    });
