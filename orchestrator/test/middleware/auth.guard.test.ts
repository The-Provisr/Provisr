...
function mockContext(
  opts: { authHeader?: string; method?: string; query?: Record<string, unknown> } = {},
): ExecutionContext {
  const req = {
    header: (name: string) =>
...
describe("AuthGuard", () => {
  ...
  describe("dev mode (DEV_USER_ID set)", () => {
    beforeEach(() => {
    ...
    it("accepts a GET with a query token (SSE via EventSource)", () => {
      const context = mockContext({ method: "GET", query: { token: "sse-token" } });
      expect(guard.canActivate(context)).toBe(true);
    });
  });
