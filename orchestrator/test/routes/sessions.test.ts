...
describe("SessionsController", () => {
  ...
  it("delegates persisted session operations to the chat service", async () => {
    ...
    await controller.list(UUID, user);
    ...
    expect(chat.listSessions).toHaveBeenCalledWith(UUID, user.userId);
    ...
    expect(chat.archiveSession).toHaveBeenCalledWith(UUID, UUID, user.userId);
  });
});
